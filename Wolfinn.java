...
public class Wolfinn {
    ...
    //JDBC Url where the database will be stored.
    ...
    public static void main(String[] args) {
        try {
            ...
            try {
                ...
                //Dropping the tables first if the tables exist
                dropTables(stmt);

                //Creating the tables
                createTables(stmt);

                //Populating the demo data into the tables
                populateData(stmt);

            } finally {

                //Clsoing the result set, statement and the connection
                close(rs);
                close(stmt);
                close(conn);
            }
            ...
            //Catching an error if anything fails and printing the trace
            oops.printStackTrace();
        }
    }

    //Populates the demo data into all the tables
    static void populateData(Statement st) throws SQLException {
        ...
        //Adding data to presidential_suite table but currently none of the dedicated staff is associated to a presidential suite in the demo data and hence nothing is added currently

        //Adding data to billing table
        ...
        //Adding data to address_gives_city table
        ...
        //Adding data to check_in_services table
        ...
        //Adding data to managed_by table
    ...
    //Drops the tables if the tables exists
    static void dropTables(Statement st) throws SQLException {

        //Dropping the tables in the order such that none of the constraints are violated
        //Also dropping the tables only if they exist
        //This is to reload the correct demo data every time the application is started
    ...
    //Creating the tables for the database in such a way that the constraints are handled properly
    static void createTables(Statement st) throws SQLException {
        ...
        //Creating presidential_suite table
        ...
        //Creating address_gives_city table
        ...
        //Creating check_in_services table
